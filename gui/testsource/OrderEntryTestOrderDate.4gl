...
MAIN
    ...
    CALL ggc.registerScenario(FUNCTION playTestOrderDate)
...
# Scenario OrderEntryPlus_mikefolcher_OrderDate_val id : 0
PRIVATE FUNCTION playTestOrderDate()

	 DISPLAY "Starting the Order Date Validation test"
    ...
    CALL ggc.assertEquals(ggc.getValue(), "", SFMT("Focused field value is not valid, expected: '', got: '%1'", ggc.getValue())) -- DATE
    CALL ggc.checkNoMessage()
    CALL ggc.checkError("Order Date is required")
